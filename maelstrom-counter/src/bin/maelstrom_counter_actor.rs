use maelstrom_counter::actor::{Event, NodeActor, StdinActor, StdoutActor};
...
async fn main() {
    ...
    let node_actor = NodeActor::new(rx_event, tx_msg, tx_event.clone());
    tokio::spawn(async move {
        node_actor.run().await;
    });

    let tx_flush = tx_event.clone();
    tokio::spawn(async move {
        let mut ticker = interval(Duration::from_millis(100));
        loop {
            ticker.tick().await;
            let _ = tx_flush.send(Event::FlushTick);
        }
    });
